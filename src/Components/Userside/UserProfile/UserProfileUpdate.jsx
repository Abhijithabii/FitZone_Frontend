...
function UserProfileUpdate() {
    ...
    const fetchUserDatas = async() => {
        await axios.get(`${BACKEND_BASE_URL}/api/updateprofile/${user.user_id}`).then(res=>{
      ...
      <Dialog open={open} handler={handleOpen}>
        ...
        <DialogBody divider>
          <div className="grid gap-6">
            <Input name='full_name' value={userDetails.full_name} label="Full Name" 
                    onChange={(e)=>{
                        setUserDetails({
                            ...userDetails,
            ...
            />
            <div className='flex'>
            <Input  name='age' value={userDetails.age}  label="Age"
                    onChange={(e)=>{
               ...
               />
            <Input name='blood_group' value={userDetails.blood_group} label="Blood Group"
                onChange={(e)=>{
                    setUserDetails({
                        ...userDetails,
                        [e.target.name] : e.target.value
            ...
            <div>
            <Input name='mobile' value={userDetails.mobile} label="Phone Number"
                onChange={(e)=>{
                  setMobileError('')
            ...
            {imagePreviewUrl ?  
                  <Avatar className='w-full h-full' src={`${imagePreviewUrl}`} alt="avatar" variant="rounded" />
              : 
                 ( image ? ( <Avatar className='w-full h-full' src={`${BACKEND_BASE_URL}${userDetails.image}`} alt="avatar" variant="rounded" /> ):(<Avatar className='w-full h-full' src="https://img.freepik.com/free-icon/user_318-159711.jpg" alt="avatar" variant="rounded" /> )   )
            }
            </div>
            </div>
          </div>
