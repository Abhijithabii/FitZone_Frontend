...
function TrainerSidebar() {
  ...
  useEffect(() => {
    ...
    const Menus = [
        ...
        { title: "Inbox", spacing: true, icon: <GiWhiteBook/>, path: "/trainer/inbox" },
  ...
  return (
    <div className='flex'>
        <div className={` shadow-2xl text-black h-screen z-auto p-6 pt-8 ${ open ? "w-72" : "w-20" } duration-300 relative `}>
            <AiOutlineMenuFold className={` bg-white text-3xl
             absolute -right-3 top-9 border-2 border-black cursor-pointer ${ !open && "rotate-180" } `} onClick={() => setOpen(!open)}/>
            ...
            />
                <h1 className={` text-black origin-left pl-5 font-medium text-2xl ${open && "mb-10"}  duration-300 ${!open && "scale-0 "} `}>FitZone Trainer</h1>
            </div>
            ...
            <ul className='pt-2'>
                { Menus.map((menu, index) => (
                    <>
                        <li key={index} className={` ${open && "border-b border-black "} text-sm flex items-center gap-x-4 cursor-pointer p-2 hover:bg-blue-300  rounded-md mt-2 ${selectedMenuItem === index ? "bg-green-500" : ""}  `}
                         onClick={ () => {
                            if (menu.path) {
                                if (menu.path === 'logout') {
                                    logoutUser()
                            ...
                            <span className=' text-black text-2xl block float-left ' > {menu.icon} </span>
                            <span className={`text-black text-base font-medium flex-1 duration-300 ${ !open && "hidden"} `} > {menu.title} </span>
                            { menu.submenu && open && (
                        ...
                        {menu.submenu && submenuOpen && open && (
                            <ul>
                                {menu.submenuItems.map((submenuItem, index) => (
                                    <li key={index} className=' text-sm text-gray-300 flex items-center gap-x-4 cursor-pointer p-2 px-5 hover:bg-light-white rounded-md mt-2 ' >
                                        {submenuItem.title}
                                    </li>
                                ))}
                            </ul>
